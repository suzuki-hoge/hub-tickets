...
class IssueTest extends FunSuite {
  ...
  test("copy - all specified") {
    val act = sut.copy("title_2", "body_2", "label_2", "assignee_2", "pipeline_2", 3, 2)

    assert(
      act == CopyRequest(
        Creation("title_2", "body_2", "label_2", "assignee_2", "pipeline_2", 2, 3)
      )
    )
  }
  ...
  test("copy - no specified") {
    ...
    assert(
      act == CopyRequest(
        Creation("title_2", None, "label_1", "assignee_1", "pipeline_1", 2, 3)
      )
    )
  }

  test("cut - all specified") {
    val act = sut.cut("title_2", "body_2", "label_2", "assignee_2", "pipeline_2", 3, 2)
  ...
  test("cut - no specified") {
    val act = sut.cut("title_2", None, None, None, None, 2, 2)

    assert(
      act.right.get == CutRequest(
        Creation("title_2", None, "label_1", "assignee_1", "pipeline_1", 2, 2),
        CutComment(1),
  ...
  test("cut - failure") {
    val act = sut.cut("title_2", None, None, None, None, 4, 2)
